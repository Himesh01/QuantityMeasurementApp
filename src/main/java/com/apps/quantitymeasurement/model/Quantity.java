    ...
    public boolean equals(Object obj) {
     ...
     public Quantity<T> convertTo(Quantity<T> val){
    	 if(val==null) throw new IllegalArgumentException("Invalid Input");
    	    // Step 1: convert val to base unit
    	    double baseValue = val.getUnit().convertToBaseUnit(val.getValue());

    	    // Step 2: convert base → current unit (this.unit)
    	    double convertedValue = this.unit.convertFromBaseUnit(baseValue);
